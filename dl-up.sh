...
# ---------- 1) 按依赖顺序启动容器 ----------
ORDER=(datalink-mysql datalink-namenode datalink-datanode datalink-metastore datalink-hiveserver2 datalink-datax)
...
for i in $(seq 1 30); do
  if docker exec datalink-mysql mysqladmin ping -uroot -p"${MYSQL_PASSWORD:?请在 datalink.conf 中设置 MYSQL_PASSWORD}" --silent &>/dev/null; then
...
for i in $(seq 1 20); do
  if docker exec datalink-hiveserver2 bash -c 'exec 3<>/dev/tcp/localhost/10000' &>/dev/null; then
...
# ---------- 5) 启动 BI (DataEase) ----------
BI_DIR="$SCRIPT_DIR/bi-docker"
if [ -f "$BI_DIR/docker-compose.yml" ]; then
  info "启动 DataEase BI 服务 (端口 8100)..."
  cd "$BI_DIR"
  docker compose up -d 2>&1 | tail -5
  info "BI看板启动中，稍后 http://localhost:8100 (admin / DataEase@123456)"
  cd "$SCRIPT_DIR"
fi

# ---------- 6) 启动应用（复用项目脚本：含建库/编译/就绪检测）----------
info "启动 DataLink 应用..."
"$SCRIPT_DIR/setup-datalink.sh"