...
public class DataDevelopmentController {
    ...
    private final DnScriptPublishMapper publishMapper;
    ...
    public R<Map<String, Object>> listTasks(
        ...
        List<DnScript> scripts = scriptMapper.selectList(scriptQw);
        ...
        List<DnSyncTask> syncTasks = syncTaskMapper.selectList(syncQw);
    ...
    private Map<String, Object> buildScriptTaskMap(DnScript s) {
    ...
    private Map<String, Object> buildSyncTaskMap(DnSyncTask t) {
        Map<String, Object> task = new HashMap<>();
        task.put("id", t.getId());
    ...
    public R<DnScript> getScript(@PathVariable Long id) {
    ...
    public R<DnScript> saveScript(@RequestBody DnScript script) {
    ...
    public R<Map<String, Object>> executeSql(@RequestBody Map<String, Object> body) {
        ...
        DnDatasource ds = resolveDatasource(datasourceId);
        if (ds == null) {
            return R.fail("没有可用的数据源，请先配置数据源");
        }

        // 灰度测试：对SELECT语句自动追加LIMIT
        String execSql = sql;
        if (rowLimit != null && rowLimit > 0) {
            execSql = applyRowLimit(sql, rowLimit);
        }

        return doExecuteSql(scriptId, execSql, ds, rowLimit);
    }

    private DnDatasource resolveDatasource(Long datasourceId) {
        if (datasourceId != null) {
            DnDatasource ds = datasourceMapper.selectById(datasourceId);
            if (ds != null) return ds;
        }
        QueryWrapper<DnDatasource> qw = new QueryWrapper<>();
    ...
    private R<Map<String, Object>> doExecuteSql(Long scriptId, String sql, DnDatasource ds, Integer rowLimit) {
    ...
    private Connection getConnection(DnDatasource ds) throws SQLException {
    ...
    public R<Map<String, Object>> publishScript(@RequestBody Map<String, Object> body) {
        ...
        DnScript devScript = scriptMapper.selectById(devScriptId);
        if (devScript == null) {
        ...
        // 创建发布记录
        DnScriptPublish publish = new DnScriptPublish();
    ...
    private Map<String, Object> runGrayscaleTest(DnScript devScript, int limit, Long prodDatasourceId) {
        Map<String, Object> result = new HashMap<>();
        ...
        DnDatasource devDs = resolveDatasource(null);
        DnDatasource prodDs = prodDatasourceId != null ?
                resolveDatasource(prodDatasourceId) : devDs;

        try {
            // Dev环境执行
            long devStart = System.currentTimeMillis();
            R<Map<String, Object>> devResult = doExecuteSql(null, applyRowLimit(sql, limit), devDs, limit);
            ...
            R<Map<String, Object>> prodResult = doExecuteSql(null, applyRowLimit(sql, limit), prodDs, limit);
    ...
    public R<Map<String, Object>> rollbackScript(@PathVariable Long publishId,
        ...
        DnScriptPublish publish = publishMapper.selectById(publishId);
        if (publish == null) {
        ...
        DnScript prodScript = scriptMapper.selectById(publish.getProdScriptId());
        if (prodScript == null) {
        ...
        if (rollbackContent == null) {
            // 从dev脚本回滚prod
            DnScript devScript = scriptMapper.selectById(publish.getDevScriptId());
            if (devScript != null) {
        ...
        // 创建回滚发布记录
        DnScriptPublish rollbackPublish = new DnScriptPublish();
    ...
    public R<Map<String, Object>> getScriptDiff(@PathVariable Long devScriptId) {
        ...
        if (devScript.getProdScriptId() != null) {
            DnScript prodScript = scriptMapper.selectById(devScript.getProdScriptId());
            if (prodScript != null) {
                result.put("prodScriptId", prodScript.getId());
    ...
    public R<Map<String, Object>> getDependencies(@PathVariable Long taskId,
        ...
        QueryWrapper<DnTaskDependency> upstreamQw = new QueryWrapper<>();
        upstreamQw.eq("task_id", taskId).eq("task_type", taskType);
        List<DnTaskDependency> upstream = depMapper.selectList(upstreamQw);

        QueryWrapper<DnTaskDependency> downstreamQw = new QueryWrapper<>();
        downstreamQw.eq("upstream_task_id", taskId).eq("upstream_task_type", taskType);
        List<DnTaskDependency> downstream = depMapper.selectList(downstreamQw);
    ...
    private List<Map<String, Object>> enrichDependencies(List<DnTaskDependency> deps, boolean isUpstream) {
        ...
        for (DnTaskDependency dep : deps) {
            ...
            if (Constants.TASK_TYPE_SCRIPT.equals(relatedTaskType)) {
                DnScript script = scriptMapper.selectById(relatedTaskId);
                ...
                DnSyncTask syncTask = syncTaskMapper.selectById(relatedTaskId);
    ...
    public R<List<DnTaskExecution>> getExecutions(@PathVariable Long taskId,
                                                   @RequestParam(defaultValue = "script") String taskType,
    ...
    public R<Map<String, Object>> overview() {
        ...
        // 各层任务数
        List<DnScript> allScripts = scriptMapper.selectList(null);
        List<DnSyncTask> allSyncTasks = syncTaskMapper.selectList(null);
        ...
        QueryWrapper<DnTaskExecution> todayQw = new QueryWrapper<>();
        todayQw.ge("start_time", today.atStartOfDay());
        todayQw.le("start_time", today.plusDays(1).atStartOfDay());
        List<DnTaskExecution> todayExecs = taskExecutionMapper.selectList(todayQw);
    ...
    private String generateVersion(Long devScriptId) {
        ...
        DnScriptPublish last = publishMapper.selectList(qw).stream().findFirst().orElse(null);
