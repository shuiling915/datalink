...
public class SystemMonitorController {
    ...
    private Map<String, Object> taskStats() {
        ...
        QueryWrapper<DlSchedulerRun> todayQw = new QueryWrapper<>();
        todayQw.eq("run_date", today).eq("run_type", "daily");
        List<DlSchedulerRun> todayRuns = schedulerRunMapper.selectList(todayQw);

        long total = todayRuns.size();
        long success = todayRuns.stream().filter(r -> r.getStatus() != null && r.getStatus() == DlSchedulerRun.STATUS_SUCCESS).count();
        long failed = todayRuns.stream().filter(r -> r.getStatus() != null && r.getStatus() == DlSchedulerRun.STATUS_FAILED).count();
        long running = todayRuns.stream().filter(r -> r.getStatus() != null && r.getStatus() == DlSchedulerRun.STATUS_RUNNING).count();
        ...
        QueryWrapper<DlScript> onlineScriptQw = new QueryWrapper<>();
        ...
        QueryWrapper<DlSyncTask> onlineSyncQw = new QueryWrapper<>();
