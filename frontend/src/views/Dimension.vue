  ...
  <el-card>
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <el-input
          v-model="keyword" placeholder="搜索维度表" class="search-input"
      ...
      <el-button type="primary" @click="resetDialog"><el-icon><Plus /></el-icon>新建维度表</el-button>
    </div>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await dimensionApi.list({ pageNum: pageNum.value, pageSize: pageSize.value, domainId: domainId.value, keyword: keyword.value })
...
const resetDialog = () => {
  ...
  isEdit.value = false
  editId.value = null
  form.value = { dimCode: '', dimName: '', domainId: null, description: '' }
}
...
const handlePublish = async (row) => {
  try {
    await dimensionApi.publish(row.id, 'admin')
...
const handleSubmit = async () => {
  if (isEdit.value) {
    await dimensionApi.update(editId.value, form.value)
    ElMessage.success('更新成功')
  } else {
    await dimensionApi.create(form.value)
