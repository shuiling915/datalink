<template>
  <div>
    <el-page-header @back="$router.back()" :title="'返回'" style="margin-bottom: 20px;">
      <template #content>
        <span style="font-size: 16px; font-weight: 500;">汇总表详情 - {{ summaryTable?.summaryName }}</span>
      </template>
    </el-page-header>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="8">
        <el-card>
          <template #header>汇总表信息</template>
          <el-descriptions :column="1" border>
            <el-descriptions-item label="汇总表编码">{{ summaryTable?.summaryCode }}</el-descriptions-item>
            <el-descriptions-item label="汇总表名称">{{ summaryTable?.summaryName }}</el-descriptions-item>
            <el-descriptions-item label="层级">{{ summaryTable?.layer }}</el-descriptions-item>
            <el-descriptions-item label="状态">
              <el-tag :type="summaryTable?.status === 'published' ? 'success' : ''">
                {{ summaryTable?.status === 'published' ? '已发布' : summaryTable?.status === 'deprecated' ? '已下线' : '开发中' }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="版本">v{{ summaryTable?.publishVersion }}</el-descriptions-item>
            <el-descriptions-item label="描述">{{ summaryTable?.description }}</el-descriptions-item>
          </el-descriptions>
        </el-card>
      </el-col>
    </el-row>
...
import { summaryTableApi } from '@/api/datamodeling'
...
const loadData = async () => {
  const res = await summaryTableApi.get(route.params.id)
  summaryTable.value = res.data.summaryTable
  fields.value = res.data.fields || []
}

const addField = () => {
  fields.value.push({ fieldName: '', fieldType: 'string', fieldComment: '', isPrimary: 0, isPartition: 0 })
}
...
const handleSaveFields = async () => {
  await summaryTableApi.saveFields(route.params.id, fields.value)
  ElMessage.success('字段保存成功')
}

const handleGenerateDDL = async () => {
  const res = await summaryTableApi.generateDDL(route.params.id)
...
const handlePublish = async () => {
  try {
    await summaryTableApi.publish(route.params.id, 'admin')
