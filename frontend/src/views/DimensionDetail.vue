<template>
  <div>
    <el-page-header @back="$router.back()" :title="'返回'" style="margin-bottom: 20px;">
      <template #content>
        <span style="font-size: 16px; font-weight: 500;">维度详情 - {{ dimension?.dimName }}</span>
      </template>
    </el-page-header>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="8">
        <el-card>
          <template #header>维度信息</template>
          <el-descriptions :column="1" border>
            <el-descriptions-item label="维度编码">{{ dimension?.dimCode }}</el-descriptions-item>
            <el-descriptions-item label="维度名称">{{ dimension?.dimName }}</el-descriptions-item>
            <el-descriptions-item label="层级">{{ dimension?.layer }}</el-descriptions-item>
            <el-descriptions-item label="状态">
              <el-tag :type="dimension?.status === 'published' ? 'success' : ''">
                {{ dimension?.status === 'published' ? '已发布' : dimension?.status === 'deprecated' ? '已下线' : '开发中' }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="版本">v{{ dimension?.publishVersion }}</el-descriptions-item>
            <el-descriptions-item label="描述">{{ dimension?.description }}</el-descriptions-item>
          </el-descriptions>
        </el-card>
      </el-col>
    </el-row>
...
import { dimensionApi } from '@/api/datamodeling'
...
const loadData = async () => {
  const res = await dimensionApi.get(route.params.id)
  dimension.value = res.data.dimension
...
const handleSaveFields = async () => {
  await dimensionApi.saveFields(route.params.id, fields.value)
  ElMessage.success('字段保存成功')
}

const handleGenerateDDL = async () => {
  const res = await dimensionApi.generateDDL(route.params.id)
...
const handlePublish = async () => {
  try {
    await dimensionApi.publish(route.params.id, 'admin')
