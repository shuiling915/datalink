<template>
  <el-card>
    ...
    <el-table :data="tableData" v-loading="loading" stripe>
      ...
      <el-table-column prop="factType" label="事实表类型" width="150">
        <template #default="{ row }">
          <el-tag>{{ row.factType === 'transaction' ? '事务事实表' : row.factType === 'periodic_snapshot' ? '周期快照事实表' : '累积快照事实表' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="250" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="$router.push(`/facttables/${row.id}`)">详情</el-button>
  ...
  <el-dialog v-model="showDialog" :title="isEdit ? '编辑事实表' : '新建事实表'" width="600px">
    <el-form :model="form" label-width="120px">
      <el-form-item label="事实表编码" required><el-input v-model="form.factCode" placeholder="如: dwd_order_detail" /></el-form-item>
      <el-form-item label="事实表名称" required><el-input v-model="form.factName" placeholder="如: 订单明细事实表" /></el-form-item>
      <el-form-item label="所属数据域" required>
        <el-select v-model="form.domainId" placeholder="选择数据域" style="width: 100%;">
          <el-option v-for="d in domains" :key="d.id" :label="d.domainName" :value="d.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="所属业务过程" required>
        <el-select v-model="form.processId" placeholder="选择业务过程" style="width: 100%;">
          <el-option v-for="p in processes" :key="p.id" :label="p.processName" :value="p.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="事实表类型" required>
        <el-select v-model="form.factType" placeholder="选择类型" style="width: 100%;">
          <el-option label="事务事实表" value="transaction" />
          <el-option label="周期快照事实表" value="periodic_snapshot" />
          <el-option label="累积快照事实表" value="accumulating_snapshot" />
        </el-select>
      </el-form-item>
      <el-form-item label="层级">
        <el-select v-model="form.layer" style="width: 100%;">
          <el-option label="DWD" value="DWD" /><el-option label="DWS" value="DWS" />
        </el-select>
      </el-form-item>
